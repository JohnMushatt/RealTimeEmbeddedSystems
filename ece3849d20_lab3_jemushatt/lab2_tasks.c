...
/*
 * Clock signal to update frequency
 */
void compute_freq(void) {
	Semaphore_post(Frequency_Sem);
}
/*
 * Task for handling 20Khz source
 */
void frequency_task(UArg arg1, UArg arg2) {
	IArg key;
	while (1) {
		Semaphore_pend(Frequency_Sem, BIOS_WAIT_FOREVER);
		key = GateTask_enter(gateTask1);
		test = (float) (gSystemClock) / (acc_interval/ acc_period);
//freq = (1.0f) / ((float) (period) / (float)(gSystemClock));
		acc_period = 0;
		acc_interval = 0;
