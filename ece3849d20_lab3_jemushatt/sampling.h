...
/**
 * Timer for pwm period globals
 */
uint32_t period;
volatile uint32_t acc_interval;
volatile uint32_t acc_period;
uint32_t prev_time;
volatile float freq;
volatile float test;
volatile int32_t current_source_period;
/**
 * End
 */
...
void frequency_count_init(void);
void copy_waveform();
void compute_fft();
