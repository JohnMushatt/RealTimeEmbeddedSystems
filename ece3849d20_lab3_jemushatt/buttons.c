 ...
 */
void process_button(uint16_t button_map) {
	...
	if (button_map) {
		if (button_map & (1 << 1)) {
			...
			if (sample_rate) {
				uint32_t pll_frequency = SysCtlFrequencyGet(CRYSTAL_FREQUENCY);
				uint32_t pll_divisor = (pll_frequency - 1)
						/ (16 * ADC_SAMPLING_RATE) + 1; //round up
				ADCClockConfigSet(ADC0_BASE,
						ADC_CLOCK_SRC_PLL | ADC_CLOCK_RATE_FULL, pll_divisor);
				ADCClockConfigSet(ADC1_BASE,
						ADC_CLOCK_SRC_PLL | ADC_CLOCK_RATE_FULL, pll_divisor);
			}
			else {
				uint32_t pll_frequency = SysCtlFrequencyGet(CRYSTAL_FREQUENCY);
		...
		if (button_map & (1 << 5)) {
			...
			PWMGenPeriodSet(PWM0_BASE, PWM_GEN_2, current_source_period);
			PWMPulseWidthSet(PWM0_BASE, PWM_OUT_5, current_source_period / 2);
		}
		if (button_map & (1 << 6)) {
			current_source_period--;
			PWMGenPeriodSet(PWM0_BASE, PWM_GEN_2, current_source_period);
...
void ButtonISR(void) {
	...
	if (presses != old_buttons) {
		Mailbox_post(mailbox0, &presses, BIOS_WAIT_FOREVER);
	}
